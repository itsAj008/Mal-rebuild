  ...
  const [active,setActive] = useState(false)
    ...
    <li className={` group ${className}`}>
        <NavLink to={link} className={({ isActive }) => (isActive ? setActive(true) : setActive(false))} style={{display:'flex'}}  >
          <div className={` text-3xl block xl:hidden p-3   ${active ? 'rounded-xl dark:bg-gray-400 bg-gray-800 dark:text-black  text-slate-200' : ' rounded-full  group-hover:rounded-xl group-hover:bg-gray-700 dark:group-hover:bg-gray-400 dark:group-hover:text-black  group-hover:text-slate-200  dark:bg-gray-800 dark:text-white'}`}>
            {icon}
          </div>
